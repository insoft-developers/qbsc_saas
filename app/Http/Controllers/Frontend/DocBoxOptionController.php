...
class DocBoxOptionController extends Controller
{
    use CommonTrait;   
    public function table(Request $request)
    {
        if ($request->ajax()) {
            $comid = $this->comid();
            $data = DocBoxOption::where('comid', $comid);
    ...
    {
        $view = 'box-option';
        return view('frontend.box_option.box_option',compact('view'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            'jenis_box' => 'required|max:100|min:3',
        ]);

        // Simpan ke database
        try {
            $input['comid'] = $this->comid();
            $input['jenis_box'] = strtoupper($request->jenis_box);
            DocBoxOption::create($input);
    ...
    {
        return DocBoxOption::destroy($id);
    }
}
